...
namespace Cactus.Blade.Configuration.ObjectFactory
{
    ...
    public sealed class ValueConverters : IEnumerable<KeyValuePair<string, Type>>
    {
        ...
        /// <summary>
        /// Attempt to get a convert function for a member (property or constructor parameter) specified by
        ...
        {
            return _converters.TryGetValue(GetKey(declaringType, memberName), out var converter)
                ? (convertFunc = converter.ConvertFunc).IsNotNull()
                : (convertFunc = null).IsNotNull();
        }
        ...
        {
            return _converters.TryGetValue(GetKey(targetType), out var converter)
                ? (convertFunc = converter.ConvertFunc).IsNotNull()
                : (convertFunc = null).IsNotNull();
        }
        ...
        private ValueConverters Add(Type targetType, Type returnType, Func<string, object> convertFunc)
        {
            ...
            if (!targetType.GetTypeInfo().IsAssignableFrom(returnType))
                throw Exceptions.ReturnTypeOfConvertFuncIsNotAssignableToTargetType(targetType, returnType);
        ...
        {
            return declaringType.IsNotNull() && memberName.IsNotNull() ? declaringType.FullName + "::" + memberName : "";
        }

        private static string GetKey(Type targetType)
        {
            return targetType.IsNotNull() ? targetType.FullName : "";
        }
        ...
        {
            get { return _converters.Select(x => new KeyValuePair<string, Type>(x.Key, x.Value.ReturnType)); }
        }
    }
}
